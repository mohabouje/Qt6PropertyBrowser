...
#include "qt6propertybrowser/qtvariantproperty.h"
#include "qt6propertybrowser/qtgroupboxpropertybrowser.h"
#include "qt6propertybrowser/qttreepropertybrowser.h"
#include "qt6propertybrowser/qtpropertybrowser.h"
...
void ObjectControllerPrivate::addClassProperties(const QMetaObject *metaObject)
{
    ...
    if (!classProperty) {
        ...
        for (int idx = metaObject->propertyOffset(); idx < metaObject->propertyCount(); idx++) {
            ...
            if (!metaProperty.isReadable()) {
                subProperty = m_readOnlyManager->addProperty(QMetaType::Type::QString, QLatin1String(metaProperty.name()));
